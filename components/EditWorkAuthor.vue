<template>
  <div class="edit-work-author">
    <label for="author">
      <input id="author" v-model="curent" type="text" name="author">
    </label>
    <div class="editBtns">
...
export default {
  ...
  data () {
    return {
      curent: this.work.authorName,
      old: this.work.authorName
    }
  },
  methods: {
    cancelEdit () {
      eventEmitter.$emit('hideWorkEdits')
      eventEmitter.$emit('editAuthor', this.curent)
    },
    sendData () {
      if (this.curent !== this.old) {
        ...
        this.$axios.$post(
          `http://${process.env.HOST}:${process.env.PORT}/api/projects/edit-author`,
          data).then(
